...
it shares with this repo is the owner. Nothing durable was watching it. The
fifteen-minute `host-watch.yml` tick already runs at $0 on a public repo and
already knows how to open ONE issue per sustained outage and mail through the
outbox, so the two sandbox origins ride it (`host_watch.py`, `ORIGINS`):

* `sandbox backend`  https://sandbox.asktherecruiter.com/healthz
* `sandbox frontend` https://asktherecruiter-sandbox-production.up.railway.app/healthz

UP means HTTP 200 within 10s with JSON `status == "ok"` and a non-empty
`version`. A timeout, any non-200 (a 4xx too, unlike the host probe), a
non-JSON body, or `status: degraded` is DOWN (`judge_healthz`, pure and
tested). The URLs live in the workflow env and nowhere else; an unset one is
exit 1, not a silent skip. Same `SUSTAINED_FAILURES` (three consecutive runs,
each retrying three times) before anything is said. Each origin has its own
sub-ledger under `origins` in `data/host_status.json` (the top level stays the
host's, which `ops_status.py [2f]` reads unchanged), its own issue marker
(`gh_fallback` now takes `marker=` / `title=`, defaulting to the host's) and
its own outbox scope. A sustained outage opens one plainly titled issue
("sandbox backend down (host stopped answering /healthz)") and mails one
outage notice through the outbox in the same tick; recovery closes the issue
and mails one RECOVERED, only if the outage was announced. A blip mails
nothing, and a recovery that lands before the outage notice ever left cancels
both (the outbox's existing `cancelled` rule).

Proven by mutation: making `status != "ok"` read as UP reddens
`test_status_degraded_is_down` and nothing else. Not proven, because it
cannot be from here: a real sandbox outage. The state machine is the host's,
under the host's tests, and a local dry run against the live origins
(`--dry-run --no-fallback`) read both UP on v1.0.944.

**Not done, deliberately:** `ops_status.py [2f]` does not print the `origins`
sub-ledgers yet. The issue and the mail are the durable channel; the offline
readout is a small follow-up.

---

## 2026-09-03: the "111 talent and employer-activity signals - Sep 3" reader digest was not generated by any code in this repository, and its two biggest "Top signals" were projections presented as current jobs — the exact bug the 2026-08-24 review fixed.

The owner asked whether that morning's edition was accurate. It was checked
line by line against `data/talent_intel.db` and the live site. Most of it
reproduces closely; two things do not, and one of those two is the same
failure mode `daily_digest.py` (2026-08-24 review, still the open item at line
173 of TECHLOG — "correct, and unwired") exists specifically to prevent.

**Nothing in this repo produced this email.** `grep -rl daily_digest
.github/workflows/` matches nothing; `daily_digest.py` has one commit (#93)
and no caller. The edition's category headings — "Leadership moves",
"Funding rounds", "Pay and benefits changes" — are not `daily_digest.py`'s
vocabulary (`SIGNALS NAMING THE MOST ROLES` / `PLANNED` / `WORKFORCE EVENTS` /
`FUNDING & LEADERSHIP`) and the reviewed module windows on `captured_at` by
deliberate design (its own comment: `published_date` carries "many source date
formats"). Re-running it for the same nominal day (`--as-of 2026-09-03`)
returns 240 signals and a 33,197 YTD — nothing like the mailed 111 / 24,896.
Whatever built the email used **`published_date`** windowing instead — direct
proof below — through code that exists nowhere in this repository, under no
test, and outside the reviewed classifier.

**The numbers, checked against `WHERE is_current=1 AND published_date>='2026-09-02' AND published_date<'2026-09-04'`:**

| claim | real (now) | verdict |
|---|---|---|
| 111 signals | 110 | does not reproduce (off by 1; see drift note) |
| "From 110 companies" | 108 distinct companies | does not reproduce (off by 2) |
| 8 verified against a primary document | 8 (`confidence='verified'`) | reproduces exactly |
| 8 state a direction of hiring | 8 (`signal_direction='hiring'`) | reproduces exactly |
| Leadership moves 52 | 52 (`pillar='leadership_change'`) | reproduces exactly |
| Pay and benefits changes 18 | 18 (`pillar='rewards_comp'`) | reproduces exactly |
| Funding rounds 28 | 26-29 depending on definition used (no single stored column means "funding round"; closest, rows with a funding field, is 27) | could not check precisely |
| YTD 24,896 (Jan 1 - Sep 3) | 24,941 on `published_date` | does not reproduce (off by 45, ~0.2%; see drift note) |
| Top 5 by size: Sibanye-Stillwater 300 AU, Yicheng 100 CM, Perplexity 98->112 (+14) SF, Stripe 578->592 (+14) NY, Two Six 57->67 (+10) | identical company, country, and delta, in this exact order, sorted by `headcount DESC` where `signal_direction='hiring'` | reproduces exactly, including the ranking |
| Indeed index 101.91 as of Aug 21, +1.91 vs baseline, unchanged from a month earlier | live page: 101.9 (+1.9), "0.0" change vs 2026-07-22, both as of 2026-08-21 | reproduces (live page rounds to 1dp; underlying figures consistent) |
| AI share 6.28% as of Jul 31, up 0.33 pt | committed `indeed_index.json`: `ai.series['2026-07-31']=6.28`, `month_ago.delta=0.33` | reproduces exactly |

**The "drift" on 111/110 and 24,896/24,941 is a real property of `published_date`
windowing, not proof of a one-off typo, and it is why `daily_digest.py` uses
`captured_at` instead.** Collectors backfill: a row discovered today can carry
a `published_date` from days or weeks ago, so a past window's total keeps
climbing after the fact and a YTD figure computed this way is never stable
enough to reconcile against a later re-query — unlike `captured_at`, which is
append-only. Whether the 111/24,896 figures were exactly right at the moment
of sending cannot be established from here; what can be established is that
they are **structurally unreproducible from now on**, which is a real defect
in the choice of windowing field regardless of whether Sep 3's specific count
was off by 0 or by 1.

**The suspicious coincidence is real and resolves clean: two different
eights, not one number rendered twice.** The 8 verified rows (Czech ARES,
Korean DART x5, SEC EDGAR, Companies House) are **all** `signal_direction =
'neutral'` — board appointments and departures, not hiring. The 8
hiring-direction rows are unrelated `reported`-confidence articles and job-
board deltas. Disjoint sets, both landing on 8 by coincidence in the real
data. Not a bug.

**"Location not recorded" on Two Six Technologies is honest, not a masked
defect.** `city`, `region`, `country`, `hq_city`, `hq_country` are all NULL on
that row. Nothing was withheld from the copy that the row actually has.

**The genuine defect: Sibanye-Stillwater's 300 and Yicheng's 100 — the two
largest "Top signals," the ones a reader's eye goes to first — are
`pipeline/count_meaning.classify()` → `PLANNED_JOBS`, not current hiring.**
Sibanye's own stored summary: *"plans to reopen the Mt Lyell copper mine ...
by 2029, creating 300 jobs"* — a mine that reopens in 2029, not a company
hiring 300 people this week. Yicheng's summary: the 100 is a **training
cohort** for a plant that has not launched; the source's actual jobs figure
(1,500) is never stored as this row's headcount at all and appears nowhere in
the edition. Both are exactly the ONCE-80,000 / GM-4,600 / Costa-Rica-500
pattern the 2026-08-24 review fixed — a projected or non-hiring figure
presented as if it were a current opening — and the reviewed classifier
**already handles both correctly** (verified live: `cm.classify()` on the
real stored rows returns `PLANNED_JOBS`, `counts_as_roles=False`, for both).
It reads `headline + summary + talent_readthrough` together specifically
because the "by 2029" and "aiming to create 1,500 jobs upon completion"
language lives in `summary`, not the headline. Two regression tests
(`tests/test_count_meaning.py::test_mine_reopening_headcount_named_only_in_summary`,
`::test_training_cohort_ahead_of_future_plant_launch`) pin this using the
real row text; reducing the classifier to headline-only text fails both
(proved by mutation before this was committed). **So the bug is not in this
repo's reviewed code — it is that whatever generated the mailed edition did
not run these two rows through it**, the same way it does not use
`daily_digest.py`'s windowing. A generator outside the reviewed pipeline can
silently reintroduce a fixed bug, and did.

**What this needs, and it is the owner's call, not a code fix:** either wire
`daily_digest.py` to whatever actually sends the subscriber mail (closing the
open item at TECHLOG:173), or make the existing, unreviewed generator route
every headcount through `pipeline/count_meaning.classify()` and window on
`captured_at`. Nothing was sent, triggered, or changed live for this check;
`data/talent_intel.db` was opened read-only throughout.
...
never dispatches a workflow, and is NOT in the talent-collect lock. **Nothing
changed in collection, the database, or the site.**

It also MERGES its own draft when all four conditions hold (owner
authorization 2026-08-14): guard passed, reviewer verdict exactly
`SELF-HEAL-REVIEW-VERDICT: LOOKS SOUND`, source/test-only diff, and a merged
preview with no failure main does not already have. Each auto-merge appends
`docs/HEALING-LOG.md` — the revert index, `git revert <merge sha>` — plus a
TECHLOG narrative, best-effort. DORMANT until the owner adds
`CLAUDE_CODE_OAUTH_TOKEN`. Kill switches: `SELF_HEAL_AUTOMERGE_DISABLED=true`
keeps the drafts, `SELF_HEAL_DISABLED=true` stops the healer. Details:
TECHLOG 2026-08-14, CLAUDE.md summary.
