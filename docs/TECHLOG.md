...
**Root cause, from the bill rather than the log.** `ask()` recorded `type(exc).__name__` and dropped the message, and the run log printed nothing more, so the refusal text is not recoverable from this run. What is: the run's whole $0.850 against a haiku-only projection of $0.894 for 164 rows, and a per-row cost ($0.0051 on a 4,993-character body) that matches haiku alone. gpt-5-mini billed nothing on 164 requests, so every request was rejected BEFORE generation: an HTTP 4xx, not a truncation and not an empty reasoning answer (those are paid for). `classify._call` sends `temperature: 0` to every model, and the GPT-5 family refuses any temperature but its default. This is the third time this repo has sent a GPT-5 model through that door and the third time it answered nothing: gpt-5-nano "40 errors, unusable" (2026-07-28), gpt-5-nano 0 of 75 (2026-08-14, above). The harness was not tested against a referee that raises on every request; the truncated-answer and non-JSON cases were.

**Fix (PR: fix/benchmark-second-referee).**
- The second referee is `openai/gpt-4o-mini`: the same vendor and the same non-reasoning request shape (temperature 0, `response_format` json_object, `max_tokens`) that the adjudicator's gpt-4o answered 16 rows with on the same day. gpt-4o itself was priced first, as instructed: $2.53 alone, $3.62 with haiku, past the $2.00 ceiling this run is approved at, and the harness refuses an estimate past its ceiling rather than clamping it. gpt-4o-mini prices at $0.15; the pair at $1.25. `classify._call` is unchanged: stripping `temperature` for one model family in the one paid door, unverified against a live key from this checkout, is not the smallest correct change.
- `no_answer(exc)` records the exception's MESSAGE, whitespace collapsed and clipped at `REASON_CHARS` (320, which keeps the whole of the 300-character upstream body `_call` already clips to), into the referee record and the parse-failure entry. A result file is diagnosable alone.
- `preflight()` asks each referee ONE one-line question (`PREFLIGHT_MAX_TOKENS = 20`) before the first body is fetched, under the same discipline as `ask` (gate read before every attempt, one request per attempt). A referee that gives no answer refuses the run with exit 1, the referee and its reason on stderr, and `grade_row` never reached. A dead referee now costs a fraction of a cent, not the sample.

**Guard.** `tests/test_extraction_benchmark.py` +5 (50): a ClassifyError's message reaches both the referee record and the parse-failure list, whitespace collapsed; a 5,000-character upstream message is clipped, not dropped; the preflight names the referee that cannot answer with its reason, reads the gate once per attempt and asks for a tiny answer; it is empty when both answer; a dead referee refuses `--grade` before `schema.connect` or `grade_row` is reached and writes no result file. The budget-stop test stubs the preflight so the suite stays offline.

**Spend.** $0.85 on the failed run (haiku, 164 rows, charged DISCRETIONARY against the owner's $20.00 grant, with a priced `extraction_benchmark` health row). Re-run priced at $1.25 before dispatch, ceiling $2.00 unchanged.

---


## 2026-09-16 - What extraction actually gets right, measured against the bodies rather than against itself

**What.** `measure_extraction.py` plus `analysis/extraction/frame.py`: a 200-row stratified benchmark that grades the six fields a record IS (`company`, the amount, the headcount, `country`, `signal_direction`, `pillar`) against the BODY of the article each row was extracted from. `signals` stores no `raw_text`, so the body is fetched at grading time through `adjudicate_guardrail.fetch_evidence` - the same door the two-referee adjudicator reads a cited source through, archived copy then publisher then Wayback, a read under `EVIDENCE_MIN_CHARS` refused rather than guessed from. Grading is the owner's 2026-09-11 rule: two referees from different vendors (`anthropic/claude-haiku-4.5` + `openai/gpt-5-mini`), agreement only. Both correct is CORRECT, both wrong is WRONG, and a disagreement, a "the source does not settle it", or a referee that gave no usable answer is UNKNOWN with its reason recorded. The denominator is correct + wrong alone and every rate carries its Wilson interval from `analysis/recall/stats.wilson`. The field definitions are imported from `classify.SCHEMA_HINT`, never retyped, including the prose paragraph under the schema that defines `signal_direction` - without it a referee marks every funding round "hiring", which is the exact mistake that paragraph was written to stop. The frame is the 8,268 current rows from the LLM extraction path; the structured collectors are excluded by name because grading them would measure a deterministic parser under a heading that says extraction. The sample, `analysis/extraction/sample-2026-09-16.json`, is drawn across region (US 32 / Europe 60 / RoW 108) x event type (funding, hiring, leadership, pay, working practices), floor of 5 per cell, language mix held inside each cell, 94 of 200 non-English by the draw-time proxy and 18 in a non-Latin script. `extraction-benchmark.yml` is dispatch-only, holds the writer lock, is queued through `drain-writers.yml`, files a priced `extraction_benchmark` health row on the DISCRETIONARY pot, and turns an incomplete stratum into a red run.

**Why.** Every measurement this repo had asked a different question. `measure_recall.py` asks what we MISSED. `ab_models.py` scores a challenger's AGREEMENT with the incumbent, which is blind when both are wrong and reads a correction as a regression. `analysis/models/gate_goldset.py` grades the gate: one yes/no question, 75 English items. None of them asked whether the fields we publish are right, and extraction writes twenty of them off a headline and a teaser. The flash-lite extraction swap on 2026-09-12 was taken on gate accuracy plus nine hand-read disagreements, which is the best evidence there was and is not a measurement of the fields readers act on.

**Cost, priced before it ran.** DISCRETIONARY, against the owner's one-time $20.00 grant. `--estimate` prices the run from the live OpenRouter model list and REFUSES a run whose estimate is past the ceiling: 200 items at 14,819 prompt characters each is $1.43 (haiku-4.5 $1.10 + gpt-5-mini $0.34). `CEILING_MAX_USD = 6.00` is a hard bar the program will not take an argument past, and a `--ceiling` above it exits 1 rather than being clamped. The gate is `adjudicate_guardrail._gate`, reused rather than re-implemented, read immediately before every request; the retry is the outer `ATTEMPTS` loop so one metered call is one request; a budget stop is UNDECIDED and leaves rows ungraded rather than failing the run.

**Two lessons taken from elsewhere, deliberately.** A parse failure is counted apart from a wrong answer and can never reach a denominator - the sibling stored 33 of 90 truncated, code-fenced answers as rejections and measured a model on them. And an unreadable body costs nothing: the fetch happens before the referees are asked, because the adjudicator once had two referees "agree" to reject a figure neither had seen, off 185 characters of a Wayback loading page.
...
**What.** `sentry_error_alert.py`: asks Sentry for issues in this project that are unresolved and first seen in the last hour, keys each cause the way `ci_alert.py` keys a CI failure (numbers normalised out of the title before hashing), and reuses `alert_state.py`'s own raise/remind/resolve machinery - pointed at its own committed ledger, `data/sentry_alert_state.json`, beside `data/alert_state.json` and never inside it, via `ALERT_STATE_PATH`. Every send goes through `ops_notify.notify`/`.resolve`, the one door; nothing here builds its own transport. When and only when a cause is genuinely NEW (not already open in the ledger), it makes exactly one OpenRouter call on `google/gemini-2.5-flash-lite`, through `OPENROUTER_OPS_KEY` - a separate key with its own $3.00/month provider limit, shared across three repositories, used for nothing else - for a three-line plain-language summary (what broke, likely cause, next step). The call is gated against `data/sentry_ops_spend.json`, this key's own committed ledger, kept apart from `data/spend_month.json` and `data/discretionary_grants.json` on purpose: those are the collection pipeline's own key and its own pot. The gate reads the cap immediately before the one HTTP request `_one_request` makes and meters immediately after; nothing here retries internally, and `summarize()` never raises - no key, a reached cap, or a raising call all degrade to `(None, <plain reason>)` and the alert still sends, saying which of the three happened. `redact()` strips an email address, a `Bearer ...` token, an `Authorization:`/`X-Api-Key:` header value and anything API-key-shaped before any error text reaches the prompt. `.github/workflows/sentry-error-check.yml` runs hourly and on `workflow_dispatch`, passing `SENTRY_AUTH_TOKEN`, `OPENROUTER_OPS_KEY`, `RESEND_API_KEY`, `OPS_MAIL_TO` and `OPS_MAIL_FROM`; it is deliberately NOT in the `talent-collect` lock (it never names the collected database's filename, which is how `tests/test_workflows.py` finds writers) and carries no concurrency group, the same reasoning as `host-watch.yml`.

**Why.** Every other alarm in this repo watches what was COLLECTED - a stale source, a wrong published figure, a red CI run. None of them watches the application itself throwing, because nothing here has ever reported to an error tracker. The owner armed two new repository secrets for exactly this: a Sentry read token and a third, hard-capped OpenRouter key that must never be confused with the collection pipeline's own allowance.

**Guard.** `tests/test_sentry_error_alert.py` (offline, network always stubbed at `fetch=`/`call=`, mail always intercepted at `opsmail.send_once` the same way `tests/test_ops_sender.py` does): no model call when nothing is new; no model call and a plain, cap-naming alert when the $3.00 cap is reached; a call that raises still sends and names the exception, spending nothing; redaction removes an email, a bearer token, an API key and a bare `X-Api-Key:` header; the same cause across two runs sends exactly once and calls the model exactly once. `python -m pytest -q`: 4941 passed (plus these 13), 53 skipped, 491 subtests passed. Two pre-existing, unrelated failures were left untouched: `tests/test_city_gazetteer.py::test_the_gazetteer_agrees_with_every_city_already_stored` (San Juan/Argentina, reproduces identically on main before this change) and `tests/test_offline_suite_is_offline.py::ProbeIsLive::test_the_probe_records_a_request_that_is_actually_made`, which fails in this sandbox because `sitecustomize.py` does not land on `PYTHONPATH` here - also reproduces on main, unrelated to this change, and not fixed by it. Not verified offline, by design: the actual Sentry response shape and the actual OpenRouter cost field on a live call, since this container has no network to either service or to OpenRouter; nor is `alert_state.claim()`'s git commit-before-send race exercised against a real concurrent push (it is the same, already-tested machinery `ci_alert.py` uses, not re-tested here).

## 2026-09-15 - A one-time owner grant has a place to live, and it funds the discretionary pot only

**What.** `data/discretionary_grants.json`, keyed by UTC month, each entry `usd`, `approved_by`, `approved_on`, `purpose`. `budget.load_grants()` validates it and raises `GrantError` on anything short of that (a malformed approval must not read as no approval); `month_grant()` returns this month's entry; `pots_for()` adds it to DISCRETIONARY and never to COMMITTED; `decide()`, `status_line()` and `report()` read it and print "grant $X (who, date, purpose)"; `spend.py --degrade` and `ops_status.py [5]` go through `pots_for`, so the gate sees it and the session reads it. Seeded with the owner's re-confirmed $20.00 (2026-09-14, two-referee adjudications and extraction benchmarks across both trackers, one time).

**Why.** The adjudicate-rows run 34899697295 spent $0.00 and returned every row UNKNOWN: `spend.py --degrade` read the discretionary pot at $1.01 of $0.89 and switched paid reads off, exactly as designed, while the owner's approval sat in a conversation no gate can read. A budget the gate cannot see is not a budget. The grant is catch-up money by the owner's own wording, so it cannot raise the collectors' stop line: a grant that could would be a way to spend past it by editing a JSON file.

**Guard.** `tests/test_budget_allocator.py`: a grant raises only discretionary; no grant changes nothing; another month's grant is ignored; eleven malformed shapes are refused; the committed file validates; the exact 2026-09-14 state (charged $1.01 against $0.89) skips without the grant and runs with it while the committed decision is untouched; every surface prints the grant. Also in this change, because run 34899697295's health row reached the committed database and reddened two guards on main: `adjudicate_rows` gets a 2400h dispatch-only leash in `staleness.py` and a `_NOT_SOURCES` reason in `tests/test_sources_page.py`; `test_a_walker_reconciles_the_month_total_when_it_is_published` is isolated from the grants file, since a grant for the current month funds the very pot that test expects to find spent.
...
## 2026-09-14 - A live row no check caught can be put to the two referees; five are queued

**What.** `adjudicate_guardrail.py --row <hash>` / `--place-row <hash>` (+ `--reason`, `--ceiling`, `--health`): opens `amount/<hash>` or `place/<hash>` through `guardrails.record(..., checks=())` so the write resolves nothing else, then runs the two-referee flow unchanged. A second prompt, `PLACE_RULES`/`PLACE_PROMPT`, asks where the source places the activity (ISO 3166-1 alpha-2, city as the source spells it); `decide_place` needs both referees on the same city and country; `apply_place` goes through `correct_city_country.reissue` (which gained a `note` parameter). A dry run removes the findings it opened. `.github/workflows/adjudicate-rows.yml` runs it on this repository's own key under the `talent-collect` lock, files a priced `adjudicate_rows` health row from the run's own `STATS["usd"]` delta, commits specs with the database, and goes red once on exit 3. One ticket is queued for the five rows in the 2026-09-13 brief (HANDOVER).
