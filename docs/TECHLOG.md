...
**1. Coverage.** `pipeline/pillar_vocab.py` is one vocabulary of five intents: work mode (RTO, hybrid, four-day week), pay and benefits (pay rise or freeze, wage agreements, bonus and benefit cuts), M&A, hiring freezes, and expansions. It covers all 16 query languages. The Google News groups (`source_registry.pillar_queries`, which each edition and the en-US anchor receive) and the free prefilter (`prefilter._PILLAR`) are both compiled from this one list, so a fetched phrase is never thrown away by the gate; `tests/test_pillar_vocabulary.py` asserts it. Before this, 30 days held 2 `rto_mandate` rows. English markets (GB IE IN AU SG ZA NG KE PH NZ CA) are now queried by NAME off the en-US anchor, 3 per run (`english_market_queries`), which reaches all of them within 4 days. The withdrawn editions stay withdrawn, for the reason recorded on `WITHDRAWN_ENGLISH_EDITIONS`. Job-ad aggregator hosts (`prefilter.JOB_AD_DOMAINS`, including the alwadifa-club.com and dimajadid.com seen on 09-22) are dropped in `run_collect.free_gate` before any paid stage.
*Cost estimate:* +28 Google News RSS fetches per run (anchor +5, 4 editions x +5, markets +3), all free. Classifier reads stay capped per run by `TIT_READTHROUGH_CAP`, so the paid ceiling is **unchanged**; the new groups compete for it. The gate sees more candidates. At about $0.00003 per gate call, even +300 candidates a run is under $0.01/run and **under $0.30/month**, offset in part by the job-ad blocklist. Precision risks not yet measured live: bare `acquires`-class M&A phrases, and Korean/Japanese M&A terms matched as substrings. Watch the rejected rows for a week.

**2. Self-heal.**
- `runner-heartbeat.yml` succeeds on the box every 30 minutes. A `pick-runner` job in collect, collect-press and collect-structured runs `runner_pick.py` on `ubuntu-latest` and moves the collector there when the last successful heartbeat is more than 2h old. The repo variable `TIT_RUNNER` forces either way.
- `catch-up.yml` runs daily and off-box. It writes a writer-queue ticket (never a direct dispatch) for any weekly or monthly structured source past cadence + 1 day or never run; today that would queue edinet_japan and opendart_korea, and singapore_acra next.
- The same job keeps one self-closing issue per configured source whose last 3 runs stored 0.
- `ops_status` now reports configured sources that never ran (`collection_schedule.never_ran`); the source list is read from the crons.
- collect-press and collect-structured get #185's exit-4 guardrail split.

**3. README.** The cadence, cost and keyed-source claims are generated by `readme_facts.py` from the crons, `spend.py` and the secret mappings; `tests/test_readme_facts.py` fails on drift.

**4. Runbook.** `docs/RUNBOOK_COLLECTION.md`.

**Owner actions:**
- Merge.
- After merge, check that `runner-heartbeat.yml` goes green on the box. Until its first success, pick-runner keeps the box.
- Decide whether google_news should stay on the EU box at all; `TIT_RUNNER=hosted` is the switch.
- The first `catch-up.yml` run may open zero-store issues for sources that are legitimately quiet. Close them with a note, or raise `ZERO_STORE_RUNS`.
